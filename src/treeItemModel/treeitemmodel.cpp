...
void TreeItemModel::setHeaderData(const QVector<QVariant> & data)
{
    d->headItem->setColumnCount(0);
    for (const QVariant & dataItem : data)
        d->headItem->appendColumn(dataItem);
}
...
bool TreeItemModel::setItemParent(const uint id, const uint pid)
{
    auto pItem = d->findItem(id);
    if (!pItem.use_count())
        return false;

    auto pParent = d->findItem(pid);
    if (!pParent.use_count())
        return false;

    auto pPrevParent = d->findItem(pItem->parentIndex());
    if (pPrevParent.use_count())
        pPrevParent->childRemove(pItem->dataIndex()); // FIXME: Must remove only copy of pointer... Must be

    pParent->childAdd(pItem);
    return true;
}

QVector<std::shared_ptr<TreeItem>> TreeItemModel::getAllChildren()
{
    QVector<std::shared_ptr<TreeItem>> result;
    std::shared_ptr<TreeItem> bufferItem;
    for (uint id = 1; id < d->currentId; id++)
    {
        bufferItem = d->findItem(id);
        if (bufferItem.use_count())
            result.push_back(bufferItem);
    }
    return result;
}
